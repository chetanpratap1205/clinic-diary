...
import { ClinicsTable, type ClinicRow } from "../../admin/_components/clinics-table";
import { ExportClinicsButton } from "../../admin/_components/export-clinics-button";
import { Building2 } from "lucide-react";
import { getAuthenticatedEmployee } from "@/lib/auth/rbac";
import { redirect } from "next/navigation";

export const dynamic = "force-dynamic";
export const metadata = { title: "My Clinics | Manager Portal" };

export default async function EmployeeClinicsPage(props: {
  ...
  // Employee Scope Condition
  const empCondition = sql`EXISTS (
    SELECT 1 FROM doctor_leads dl 
    WHERE dl.phone = ${clinics.phone} AND dl.assigned_employee_id = ${emp.employeeId}
  )`;

  // Dynamic filter building
  let condition: any = empCondition;
  ...
  if (specialtyFilter && specialtyFilter !== "all") {
    condition = and(condition, eq(clinics.specialty, specialtyFilter));
  }
  
  if (tab === "active") {
    condition = and(condition, eq(subscriptions.status, "active"));
  } else if (tab === "past_due") {
    condition = and(condition, eq(subscriptions.status, "past_due"));
  } else if (tab === "cancelled") {
    condition = and(condition, eq(subscriptions.status, "cancelled"));
  } else if (tab === "trial") {
    condition = and(condition, isNull(subscriptions.status));
  } else if (tab === "at_risk") {
    condition = and(
      condition,
  ...
  const countsResult = await db.execute(sql`
    ...
    WHERE EXISTS (SELECT 1 FROM doctor_leads dl WHERE dl.phone = c.phone AND dl.assigned_employee_id = ${emp.employeeId})
  `);
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-2 border-b border-slate-200/60">
        <div>
          <div className="flex items-center gap-2">
            <h2 className="text-2xl font-bold tracking-tight text-slate-900">My Clinics</h2>
          ...
          <p className="text-slate-500 mt-1 text-sm">
            Monitor the health and subscriptions of practices you converted.
          </p>
        </div>
        <div>
