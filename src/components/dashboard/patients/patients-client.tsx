    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSearchSubmit} className="relative max-w-2xl mx-auto sm:mx-0 group">
        <div className="absolute inset-y-0 left-4 sm:left-5 flex items-center pointer-events-none">
          <Search strokeWidth={2} className="w-4 h-4 sm:w-5 sm:h-5 text-slate-400 group-focus-within:text-sky-500 transition-colors" />
        </div>
        <input
          type="text"
      ...
      <div>
        {patients.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-5 auto-rows-fr opacity-100 transition-opacity duration-200" style={{ opacity: isPending ? 0.6 : 1 }}>
            {patients.map((patient) => (
              <Card key={patient.id} className="group relative h-full flex flex-col border-slate-200/60 shadow-sm hover:shadow-xl hover:shadow-sky-500/10 transition-all duration-300 hover:-translate-y-1 hover:border-sky-300/60 bg-white/80 backdrop-blur-sm overflow-hidden rounded-2xl">
                
                {/* Main Card Content (Clickable) */}
                <Link
                  href={`/dashboard/patients/${patient.id}`}
                  className="p-4 sm:p-5 flex flex-col flex-1 z-10 pb-16 sm:pb-5"
                >
                  <div className="flex items-start gap-3.5">
                    {/* Squircle Avatar */}
                    <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-[1rem] sm:rounded-[1.2rem] bg-gradient-to-br from-slate-50 to-slate-100 flex items-center justify-center flex-shrink-0 shadow-inner border border-slate-200/80 group-hover:from-sky-50 group-hover:to-blue-50 transition-colors duration-300 relative overflow-hidden">
                      <div className="absolute inset-0 bg-white/40"></div>
                      <span className="text-slate-700 group-hover:text-sky-700 font-bold text-lg sm:text-xl relative z-10">
                    ...
                    <div className="min-w-0 flex-1 pt-0.5">
                      <p className="font-bold text-slate-900 text-sm sm:text-base truncate group-hover:text-sky-700 transition-colors">
                        {patient.name}
                      </p>
                      <div className="flex items-center gap-1.5 mt-1 text-slate-500">
                        <Phone strokeWidth={2} className="w-3.5 h-3.5 text-slate-400 shrink-0" />
                        <span className="text-xs font-semibold text-slate-600 truncate">{patient.phone}</span>
                      </div>
                      {/* P1: Pending follow-up indicator */}
                      {Number(patient.pendingFollowUps) > 0 && (
                        <div className="flex items-center gap-1.5 mt-1.5">
                          ...
                          <span className="text-[11px] font-bold text-amber-700">
                            {Number(patient.pendingFollowUps)} follow-up{Number(patient.pendingFollowUps) > 1 ? 's' : ''} pending
                          </span>
                        </div>
                      )}
                    </div>
                  ...
                  <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between gap-2">
                    <div className="flex items-center gap-1 text-indigo-700 bg-indigo-50/80 px-2 py-0.5 rounded-lg border border-indigo-100/50">
                      <Activity strokeWidth={2} className="w-3 h-3 flex-shrink-0" />
                      <span className="text-[11px] font-bold">
                        {patient.visitCount} Visit{patient.visitCount !== 1 ? 's' : ''}
                      </span>
                    </div>
                    {patient.age ? (
                      <p className="text-[11px] text-slate-500 font-bold bg-slate-50 px-2 py-0.5 rounded-lg border border-slate-100">
                        {patient.age} yrs
                      </p>
                    ) : (
                ...
                {/* Quick Actions Bar (Always visible on mobile, hover-revealed on desktop) */}
                <div className="absolute bottom-0 left-0 right-0 p-2 sm:p-2.5 bg-white/95 backdrop-blur-md border-t border-slate-100 z-20 flex items-center justify-around gap-1 sm:translate-y-full sm:opacity-0 sm:group-hover:translate-y-0 sm:group-hover:opacity-100 transition-all duration-300 ease-out shadow-sm">
                   <a 
                     href={`tel:${patient.phone}`} 
                     className="flex-1 flex items-center justify-center gap-1 p-1.5 rounded-xl text-slate-600 hover:text-sky-600 hover:bg-sky-50 transition-colors font-bold text-[11px]"
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2 pt-4 sm:pt-6">
          ...
          <div className="flex items-center gap-1 px-4 py-2 rounded-xl bg-white border border-slate-200 shadow-sm text-xs sm:text-sm font-medium text-slate-700">
