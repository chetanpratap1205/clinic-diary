...
export function PatientInstallButton({ clinicName, className = "" }: { clinicName: string, className?: string }) {
  const [deferredPrompt, setDeferredPrompt] =
    useState<BeforeInstallPromptEvent | null>(null);
  const [isInstalled, setIsInstalled] = useState(false);
  ...
  const displayName = clinicName.length > 15 ? "App" : clinicName;

  return (
    <button
      onClick={handleInstall}
      className={`group relative flex items-center gap-2 overflow-hidden rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 shadow-sm transition-all hover:border-slate-300 hover:bg-slate-50 hover:shadow active:scale-95 ${className}`}
      title={`Install ${clinicName} App (< 1MB)`}
    >
      <div className="absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-slate-100 to-transparent transition-transform duration-1000 group-hover:translate-x-full" />
      <div className="relative flex h-5 w-5 items-center justify-center rounded-full bg-slate-100 text-slate-600 transition-colors group-hover:bg-slate-200 group-hover:text-slate-800">
        <Download className="h-3 w-3" />
      </div>
      <div className="relative flex flex-col items-start leading-none">
        <span className="text-[10px] uppercase tracking-wider text-slate-400">Get App</span>
        <span className="truncate">Install {displayName}</span>
      </div>
      <div className="relative ml-1 hidden rounded-full bg-slate-100 px-1.5 py-0.5 text-[9px] font-bold text-slate-500 sm:block">
        &lt; 1MB
      </div>
    </button>
  );
